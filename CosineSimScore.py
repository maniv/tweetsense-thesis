...
class CosineSim:
    ...
    @staticmethod
    def computeCosineSim(mongodbobject,tweetcollec,outputcollec,query_set,tweettextmap):
        ...
        for hashtag , document in tweettextmap.iteritems():
            ...
            mongodbobject.insertCollection(simscoremap,outputcollec)
            #simscoremap[tweetid] = cosscore[counter]
            print simscoremap
            counter+=1

        # for tweets in mongodbobject.selectCollection(tweetcollec):
        ...
        #                        "simscore": simscore}
        #         mongodbobject.insertCollection(tagsimscore,outputcollec)

        print "Done"

    #Get Tweettextwithout@mentions from Collection # Mani System
    @staticmethod
    def get_AllParsedTweeTextAndHashTagMap(mongodbobject, inputcollec):
    ...
    @staticmethod
    def computeCosineSimBasedonTweetId(mongodbobject,tweetcollec,outputcollec,query_set,tweettextmap):
    ...
    @staticmethod
    def get_AllParsedTweeTextAndIdMap(mongodbobject, inputcollec):
        atPattern = re.compile(r'@([A-Za-z0-9_]+)')
        hPattern = re.compile(r'#(\w+)')
        urlPattern = re.compile(r"(http://[^ ]+)")
        notLetterPattern = re.compile('[^A-Za-z0-9]+')
...
# outputcollec =  "tweetid_simscore"
# tweetcollec = "docs_text"

mongodbobject = MongoDB("ctxbot")
inputcollec =  "ukcandidateset"
outputcollec1 = "uk_cossimscore"
outputcollec2 = "uk_cossimscore_tweetid"
outputcollec3 = "uk_cossim_tweetid_max"

# mongodbobject = MongoDB("ctxbot")
# inputcollec =  "uk_tag_simscore"
# outputcollec1 =  "uk_simscore_trending"
# outputcollec2 = "uk_simscore_summation"

# CosineSim.constructMaxSimScoreCollec(mongodbobject,outputcollec2,outputcollec3)

query_set = ["I would love a go at Luge Or Ski Jump Looks awesome"] #Query

tweetidmap = CosineSim.get_AllParsedTweeTextAndIdMap(mongodbobject,inputcollec)
CosineSim.computeCosineSimBasedonTweetId(mongodbobject,inputcollec,outputcollec2,query_set,tweetidmap)

#query_set = ["database index"] #Query
# tweettextmap = CosineSim.get_AllParsedTweeTextAndIdMap(mongodbobject, inputcollec)
# CosineSim.computeCosineSim(mongodbobject,inputcollec,outputcollec,query_set,tweettextmap)

#CosineSim.computeHashtagSimScoreSummation(mongodbobject,inputcollec,outputcollec2)
#CosineSim.computeHashtagSimScoreTrending(mongodbobject,inputcollec,outputcollec1)
