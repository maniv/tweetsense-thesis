...
class MongoDB:
    ...
    #Select * Collection
    def SortedCollection(self, name=""):
        list = []
        for item in self._db[name].find().sort("regscore" ,-1):
            list.append(item)
        return list
    ...
    #Select * Collection
    def selectRegscoreCollection(self, name=""):
        list = []
        for item in self._db[name].find().sort({"regscore" : -1}):
            list.append(item)
        return list
    ...
    #Select Specific users tweets
    def selectFidTweets(self, fid, name=""):
        list = []
        for item in self._db[name].find({"fid" : fid}):
            list.append(item)
        return list
